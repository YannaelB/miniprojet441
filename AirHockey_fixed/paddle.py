...
class Paddle:
    ...
    def move_sensor(self, y_position):
        if y_position < 800:
            self.y = self.y
        elif y_position >5000:
            self.y = self.y
        else:
            self.y = int(((y_position-900 )/2500)*const.HEIGHT*0.90)
        print(" HERE move sensor self.y = ", self.y )

    def move_sensor_fluid(self, y_position):
        if self.y > (y_position-900)/2300:
